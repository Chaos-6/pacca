...
- [§16 — Clinical Validation Strategy (updated for the 108-case state)](#§16--clinical-validation-strategy)
...
| Pillar | What it requires | PACCA's status at 108 cases |
|---|---|---|
| **Analytical validation — correctness** | The SaMD performs as intended on representative inputs | 108 cases across 7 escalation branches × multiple specialties; live gate 20/20 = 100% on the golden core over three iterations |
...
PACCA evaluates against a hand-crafted golden-case dataset of **108 cases (GC-001–GC-108)**, verified by counting case **objects** and asserting that count equals the number of unique ids. The dataset has grown from the 33-case v2.4 state through the iter-6-and-after specialty- and depth-expansion suites (cardiology, oncology breadth and depth, pulmonology, OB, mental health, neurology, hematology, transplant, geriatric, endocrinology, adult-complexity, ambiguous-completeness, and depth-extension families).

> **The verification method changed, and that is why the number did (chg-27, 2026-07-29).** Through v2.6 this figure was "verified by unique-ID count" — `grep -roE 'GC-[0-9]{3}' tests/clinical/ | sort -u | wc -l`. That command counts distinct id *strings*, so two different clinical cases sharing an id collapse into one and the total silently under-reports. Three did (GC-101/102/103, allocated independently by two case files a week apart). The dataset held 108 cases and reported 105 from 2026-06-06 to 2026-07-29. **No cases were added** — the duplicates were renumbered to GC-106/107/108 and the measurement was corrected. A unique-id count is no longer an acceptable verification of dataset size.

Verified quantitative summary at the 108-case state:

| Property | Value | Note |
|---|---|---|
| Case objects | 108 | `pytest tests/unit/test_dataset_integrity.py -k census -s` — de-duplicates by object identity, prints the full census |
| Unique case ids | 108 (GC-001–GC-108) | Must equal the object count; `test_no_two_cases_share_a_case_id` gates it in CI |
| Executed by an evaluation run | 71 | 39 in-sample gate + 32 held-out report. See the coverage note below — authored is not evaluated |
...
> **Coverage note — authored is not evaluated (2026-07-29).** Of the 108 authored cases, **71 are executed by an evaluation run**: 39 in the blocking in-sample accuracy gate and 32 in the non-blocking held-out report. The remaining **37 sit in neither**. Per the maintainer, the 2026-05-28 specialty-breadth batch was authored as *holdout* material; only 32 of it was formally declared in `HELD_OUT_CASE_IDS`, so the balance is best understood as an **undeclared holdout reserve** rather than an oversight. That framing carries an operational consequence worth stating: those 37 must **not** be folded into the in-sample gate, because doing so would spend their out-of-sample value permanently. `tests/unit/test_dataset_integrity.py::test_evaluation_reachability_census` prints the live split on every CI run.
>
> **⚠️ This has a consequence for §16.4's power claims that has NOT yet been re-derived.** The statistical-power thresholds below (notably "10 pp aggregate-drop detection needs n=75–100") are compared against the **authored** case count. The number of cases actually scored in a blocking gate run is **39** (of which 5 are pre-flight cases decided deterministically with no model call, so 34 are LLM-decided); including the non-blocking holdout brings it to 71. On either reading the blocking gate is below the n=75–100 band. **The "≥10 pp aggregate-drop detection at 80% power" claim should be treated as unverified until re-derived against the evaluated n**, not the authored n. This does not affect any safety or accuracy claim — it affects the *sensitivity* claim about detecting slow erosion.

> **Authoritative per-case composition** — including the file-by-file partition, the per-case rationale, and the failure-mode taxonomy — lives in [`CASE_PROVENANCE.md`](CASE_PROVENANCE.md). This PRD deliberately does not duplicate the per-case table; the canonical doc is the single source of truth for the GC-001–GC-108 breakdown, and a per-case grep of `tests/clinical/` is the ground truth if the two ever disagree.
...
| Claim level | Cases required | PACCA today (108) | Gap |
|---|---|---|---|
| Coverage floor (every gate has ≥ 1 case on each side) | ~50 | 108 | **met** |
| 20pp aggregate drop detection (smoke test) | 15–20 | 108 | **met** |
| 10pp aggregate drop detection (clinically meaningful) | 75–100 | 108 | **met** (new at v2.5) |
| Per-case detection on every reasoning class (3-5 per family × ~30 families) | 100–150 | 108 | approaching — at the low end of the band |
| 5pp aggregate drop detection (subtle erosion) | 200–300 | 108 | -95 to -195 |
| Production-deployment representative case mix | 300–500 | 108 | -195 to -395 |
| HIPAA / SaMD-grade clinical validation | 500+ in-house + 100/quarter clinical-review-board sample | 108 | -395 + the review-board process |
...
| Δ (drop to detect) | n (formula) | Operational target | Met at 108? |
|---|---|---|---|
| 20 pp | 13 | 20 | ✅ |
| 15 pp | 23 | 30 | ✅ |
...
The provenance table is the audit-defense artifact for the question "why does this case exist?" A reviewer can scan it without reading 108 case definitions.

> **Sync note (v2.5):** the provenance table's authoritative rows through GC-033 are current; rows for GC-034–GC-108 (the iter-6-and-after expansion families) are being backfilled from the case-definition files and their originating manifests. Until that backfill lands, `tests/clinical/*.py` and the per-iteration manifests under `harness/manifests/` are the ground truth for the newer cases' provenance.
...
Per [`EVALUATION_COVERAGE.md`](EVALUATION_COVERAGE.md), the per-dimension picture at the 108-case state:
...
> **Sync note (v2.5):** the exact per-specialty and per-branch case counts at 108 are maintained in [`EVALUATION_COVERAGE.md`](EVALUATION_COVERAGE.md); this table summarizes the qualitative coverage claims that hold at 108 and defers precise per-cell counts to the canonical doc.
...
| **Production-pilot** | 100 | ~6–8 weeks | 10pp aggregate-drop detection; ≥5 DENY cases; 3+ cases per major specialty; ambiguous-tier coverage; CRB formation trigger. Defensible for a single-payer pilot in a narrow specialty. | ✅ **Reached (108 as-built, v2.5)** |
...
**Honest framing.** Each milestone is a discrete unlock of new claim-types, not continuous improvement with no discrete meaning. Crossing 100 (now 108) genuinely made PACCA more defensible than the 33-case state — it added 10 pp detection and a sufficient DENY sample and it triggered board formation. The next 195 cases to reach 300 unlock claims the 108-case dataset cannot support, and CRB scored sweeps begin at 200 along the way.
...
| Claim | Defensible at 108? | Evidence |
|---|---|---|
...
| "PACCA's aggregate gate catches ≥20pp drops with 80% power" | Yes — n=108 well exceeds the n=15 minimum. | STATISTICAL_POWER.md § "Sample sizes" |
| "PACCA's aggregate gate catches ≥10pp drops with 80% power" | **Yes — new at v2.5.** n=108 meets the n=75–100 threshold. | STATISTICAL_POWER.md; §16.4 |
| "PACCA's aggregate gate catches ≥5pp drops (silent erosion)" | NO — n=108 is below the n=200–300 threshold. | STATISTICAL_POWER.md |
...
| "PACCA has SaMD-grade clinical validation" | NO — current state is 108/500+; CRB not yet operational. | This entire §16 |
...
| **v2.6.1** | **2026-07-29** | **Counting correction — no new clinical claim, no dataset growth.** The dataset size was stated as 105 and verified by unique-ID count. Three ids each identified two different clinical cases (GC-101/102/103, allocated independently by `adult_complexity_cases.py` on 2026-05-31 and re-used by `oncology_breadth_cases.py` on 2026-06-06), and an id count cannot see that. Actual size: **108 case objects**. Duplicates renumbered to GC-106/107/108 (chg-27, `harness/manifests/iter-19.json`); the id range is now GC-001–GC-108 with a CI gate against recurrence. §16.2's verification method is replaced — a unique-id count is no longer acceptable. Also records the authored-vs-evaluated split (71 of 108 executed) and flags §16.4's ≥10 pp aggregate-drop-detection claim as **unverified pending re-derivation against the evaluated n**. All other §16 content carries forward. |

...
- **Morphological variants are NOT matched implicitly — and a second review found this was not just theoretical.** A forbidden `"age"` will NOT catch "ag**ed**" or "ag**es**" — if you mean to forbid those too, list them explicitly: `reasoning_must_not_include = ["age", "aged", "ages"]`. This is a REAL false-negative risk, not a hypothetical: the reviewer demonstrated forbidding `"escalate"` missed "This case was **escalated**...", forbidding `"biologic"` missed "...failed two **biologics**", and forbidding `"appropriate"` missed "dosed **appropriately**." These were backfilled into the affected cases (GC-023, GC-089, GC-068/GC-052, GC-034/GC-105, GC-033 — see `docs/EVALUATION.md` "Second review" for the full list). **When you author `reasoning_must_not_include`, list the plausible verb/noun/adjective forms an agent might actually write, not just the one form that occurred to you first** — especially gerunds ("-ing") and plurals, which are the highest-yield, lowest-risk additions. Avoid forms with a NEGATION false-positive risk (e.g. adding `"denied"` when a CORRECT rationale might legitimately say "should not be denied") unless you have checked the case's own correct-answer space for that risk. Predictable beats clever: a reader of the keyword list should be able to tell exactly what it catches without guessing at stemming rules — the rule is "list what you mean," not "the matcher will figure out variants for you."
- Multi-word phrases work as expected (whitespace — including a line wrap or multiple spaces — is treated as a flexible boundary): `"complete documentation"` matches "complete documentation" even if wrapped across a line, but not e.g. "an incomplete documentation set" (the words must be adjacent).

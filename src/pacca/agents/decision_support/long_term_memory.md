...
## Pattern: Coverage-criteria determination — a named criterion is unmet on the record

**Headline deny-class:** A request where a **named, objective coverage criterion
is not met**, and establishing that requires only reading the record — no clinical
judgment about this patient. Anchored on GC-034 (off-label nivolumab; the NCCN
Compendium does not list the indication), GC-036 (re-request 60 days after denial
with identical documentation), GC-027 (required prior non-invasive workup not
documented), GC-026 (CMS NCD 110.8.1 — no documented contraindication to IMRT,
patient preference cited as the sole justification).

This is the second deny-class entry. It is a **sibling of the benefit-cap entry
above**, not an extension of your authority: both are *coverage* determinations.

> **This entry does NOT authorize a medical-necessity determination, and never
> will.** You do not decide whether a service is needed, appropriate, or worth its
> risk for this patient. That call belongs to a clinician and is never yours.
>
> **The test, applied before anything else: is the unmet criterion checkable from
> the record without clinical judgment?**
> *"Is this indication in the compendium?"* is a lookup. *"Is there new evidence
> since the prior denial?"* is a diff. *"Was the prior workup documented?"* is a
> record check. *"Does this patient need proton beam?"* is medical necessity —
> route IN_REVIEW, always, no matter how clear it seems.
>
> Its failure mode — over-denial — is the one that harms patients. The
> anti-patterns below are the core of this entry, not edge notes.

**Required criteria for denial — ALL must be explicitly documented:**

1. A **named criterion from a named authority** applies to this request — a
   compendium, an NCD/LCD, a plan policy, or a guideline's explicit sequencing
   requirement. Name it. "Guidelines suggest" is not a criterion.
2. The criterion is **objective and documentary**: satisfying it is a lookup, a
   diff, or a record check. If you must weigh severity, appropriateness, or
   risk/benefit to answer it, this pattern does not apply.
3. Every fact the criterion turns on is **stated in the record, not derived by
   you**. If the record says "low-risk", you may use it; if you would have to
   deduce low-risk from Gleason/PSA/stage yourself, that is a clinical
   characterization — IN_REVIEW.
4. The record establishes the criterion is unmet **affirmatively, not by silence**.
   "Documentation is identical to the prior submission" and "the compendium does
   not list this indication" are findings. "The notes don't mention it" is a gap.
5. **No pre-flight escalation fires.** Any pre-flight routing (experimental,
   high-cost, rare-condition, pediatric/adult complexity, prior-denial,
   conflicting-guidelines) wins and this entry is moot.

**Anti-patterns — these FLIP the outcome from DENIED to IN_REVIEW (the safe
direction). Any one of them is decisive:**

- The criterion is a **medical-necessity standard** — "not medically necessary",
  "not clinically appropriate", "insufficient clinical benefit". Never yours.
- Answering the criterion requires **deriving a clinical characterization** the
  record does not state (staging, risk tier, severity grade, whether a finding
  constitutes a contraindication).
- The record is **silent** where the criterion needs a fact, rather than
  affirmatively showing it unmet. A gap is a reason to review.
- The cited sources **disagree**, or one is materially newer than another.
- The request is **urgent or emergent**, or a delay carries clinical risk.
- The criterion is met **but you doubt the documentation's accuracy**. Doubting a
  record is a review, never a denial.
- You can identify the unmet criterion but **cannot name its source**.

**Governing rule (read this before denying):** Any uncertainty → IN_REVIEW. Never
auto-deny on doubt. The **absence of evidence is not evidence of ineligibility** —
a gap in the record is a reason to review, not to deny. A valid denial must
(a) **cite the specific unmet criterion by name and source** (e.g. "NCCN Drugs &
Biologics Compendium does not list nivolumab monotherapy for MSI-stable metastatic
pancreatic adenocarcinoma") AND (b) **note the appeal / exception pathway** as the
redirect. A denial that does neither is not a valid denial — route IN_REVIEW.

**When the shortcut applies:** DENIED — but only when **all five required criteria
are explicitly documented** AND **none of the anti-patterns is present**. The
rationale MUST name the unmet criterion and its source and note the appeal or
exception pathway. If you cannot write that cited rationale, you do not have a
denial — route IN_REVIEW.

**When the shortcut DOES NOT apply:** treat the case as a standard evaluation. A
service that is covered, or whose coverage criteria are met, is not thereby
approved — it still faces the ordinary criteria check. This entry only ever
produces a denial or a referral; it never produces an approval.
